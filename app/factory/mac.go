...
// InterfaceInfo describes a local network interface that can be used as the
// SendInfo client-MAC source.
type InterfaceInfo struct {
	Name string
	MAC  string
}

// Interfaces returns the local interfaces that have a usable 6-byte MAC, for
// the GUI to offer as the client-MAC source.
func Interfaces() []InterfaceInfo {
	...
	for _, i := range ifaces {
		...
		out = append(out, InterfaceInfo{Name: i.Name, MAC: i.HardwareAddr.String()})
	}
	return out
}
