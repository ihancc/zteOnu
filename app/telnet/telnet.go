...
// Exec sends a shell command, waits for the shell prompt, and returns the
// device output with the echoed command and telnet control bytes stripped. It
// is the exported form of runOutput for callers that drive the shell directly
// (see the one-click provisioning flow).
func (t *Telnet) Exec(cmd string) (string, error) {
	return t.runOutput(cmd, readTimeout)
}

// ExecTimeout is Exec with a caller-chosen timeout, for commands that can take
// longer than readTimeout to return the shell prompt (e.g. sdefconf writing
// flash).
func (t *Telnet) ExecTimeout(cmd string, timeout time.Duration) (string, error) {
	return t.runOutput(cmd, timeout)
}

// ExecReboot sends a command that makes the device reboot (or otherwise drop
// the session) and blocks until the device closes the connection - like Reboot,
// but for an arbitrary command and with a caller-chosen close timeout. Config
// commands such as `upgradetest sdefconf <id>` write flash before rebooting, so
// a longer timeout than the plain reboot is appropriate.
func (t *Telnet) ExecReboot(cmd string, timeout time.Duration) error {
	if err := t.sendCmd(cmd); err != nil {
		return err
	}
	return t.waitForClose(timeout)
}
